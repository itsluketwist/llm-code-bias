"""Method to get LLM recommendations for tasks."""
...
from src.prompts import LANGUAGE_RANK_PROMPT, LIBRARY_RANK_PROMPT


@experiment
def get_llm_code_recommendations(
    ...
    # use a different template depending on the bias type
    if bias_type == BiasType.LANGUAGE:
        _template = LANGUAGE_RANK_PROMPT
    elif bias_type == BiasType.LIBRARY:
        _template = LIBRARY_RANK_PROMPT
    else:
    ...
    for _id, _idx in tqdm(tasks):
        ...
        prompt = _template.format(task=dataset[_id])  # build prompt
